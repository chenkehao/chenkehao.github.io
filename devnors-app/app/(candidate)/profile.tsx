...
export default function CandidateProfileScreen() {
    ...
    Alert.alert('切换身份', '确定要切换为招聘方身份吗？', [
      { text: '取消', style: 'cancel' },
      {
        ...
        onPress: async () => {
          setSwitching(true);
          setSwitchMsg('正在切换身份...');
          const result = await setUserRole('employer');
          if (result.success) {
            setSwitchMsg('切换成功');
            setTimeout(() => {
              setSwitching(false);
  ...
  const menuItems: {
    ...
    { icon: 'sparkles-outline', label: 'AI 助手', onPress: () => router.navigate('/(candidate)/ai' as `/${string}`) },
    { icon: 'people-outline', label: '邀请好友', route: '/(common)/invite' },
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.light.bgSecondary }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <Card variant="elevated" style={{ marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <View style={{ marginRight: 14 }}>
              <Avatar uri={user?.avatar_url} name={user?.name} size={60} />
            </View>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6, gap: 6, flexWrap: 'wrap' }}>
                ...
                <View style={{ backgroundColor: COLORS.light.bgSecondary, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4 }}>
                  <Text style={{ fontSize: 11, color: COLORS.light.muted, fontWeight: '500' }}>求职者</Text>
                </View>
                {user?.is_verified && (
        ...
        >
          ...
          <Text style={{ fontSize: 15, fontWeight: '500', color: COLORS.primary }}>切换为招聘方</Text>
        </TouchableOpacity>
