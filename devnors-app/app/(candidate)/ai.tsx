...
export default function CandidateAIScreen() {
  ...
  const { data: memories = [] } = useQuery({
    queryKey: ['memories', user?.id, 'candidate'],
    queryFn: () => getMemories(user?.id || 0, 'candidate'),
    enabled: !!user?.id,
  });
  ...
  const getWelcomeMessage = useCallback((): ChatMessage => {
    ...
    return {
      id: 'welcome', role: 'assistant',
      content: `你好${user?.name ? ' ' + user.name : ''}！我是 Devnors AI 助手。\n\n我可以帮你：\n- **优化简历** 并给出专业建议\n- **推荐匹配职位** 和薪资分析\n- **模拟面试** 练习\n- **智能投递** 自动匹配并投递\n- 解答任何 **职场问题**\n\n你也可以点击右上角的任务列表，选择特定任务进行对话。`,
      timestamp: new Date().toISOString(),
    };
  ...
  const buildMemoryContext = useCallback(() => {
    ...
    const ctx = memories
      .filter((m: Memory) => m.importance === 'high' || m.type === 'requirement' || m.type === 'skill')
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      let context = '';
      if (selectedTodo) {
        context += `当前任务: ${selectedTodo.title} (类型: ${selectedTodo.todo_type || '通用'}, 进度: ${selectedTodo.progress}%)`;
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {selectedTodo && (
              <Text style={{ fontSize: 11, color: COLORS.light.muted, marginTop: 1 }}>进度 {selectedTodo.progress}%</Text>
            )}
          </View>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <ModelSelector
            ...
            accountTier={(user?.account_tier?.toUpperCase() || 'FREE') as AccountTier}
            onSelect={setCurrentModel}
          />
          <TouchableOpacity
            onPress={() => setShowTaskPanel(true)}
            style={{ width: 34, height: 34, borderRadius: 10, backgroundColor: COLORS.light.bgSecondary, alignItems: 'center', justifyContent: 'center' }}
          >
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ChatMessageComponent
              message={item}
              onActionPress={(msg) => sendMessage(msg)}
        ...
        <View
          style={{
            ...
            backgroundColor: COLORS.light.bg, borderTopWidth: 1,
            borderTopColor: COLORS.light.borderLight, gap: 8,
          }}
        >
          <TextInput
            style={{
              flex: 1, backgroundColor: COLORS.light.bgSecondary, borderRadius: 22,
              paddingHorizontal: 18, paddingVertical: 10, fontSize: 15,
              maxHeight: 100, color: COLORS.light.text,
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              backgroundColor: input.trim() ? COLORS.primary : COLORS.light.disabled,
